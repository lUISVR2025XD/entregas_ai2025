...
const ShoppingCart: React.FC<ShoppingCartProps> = ({ isOpen, onClose, cart, business, userLocation, onUpdateQuantity, onRemoveItem, onPlaceOrder }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-end z-50">
            <div className="bg-white dark:bg-gray-800 w-full max-w-lg h-full shadow-2xl flex flex-col transform transition-transform duration-300 translate-x-0">
                ...
                {cart.length > 0 && business ? (
                    <div className="p-4 overflow-y-auto flex-grow">
                        <h3 className="font-semibold mb-2 text-lg">De: <span className="text-orange-500">{business.name}</span></h3>
                            ...
                            {cart.map(item => (
                                <li key={item.product.id} className="py-3 flex items-center">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => onUpdateQuantity(item.product.id, item.quantity - 1)} className="p-1 rounded-full bg-gray-200 dark:bg-gray-700"><Minus size={16}/></button>
                                        <span>{item.quantity}</span>
                                        <button onClick={() => onUpdateQuantity(item.product.id, item.quantity + 1)} className="p-1 rounded-full bg-gray-200 dark:bg-gray-700"><Plus size={16}/></button>
                                    </div>
                                    <span className="w-20 text-right font-semibold">${(item.product.price * item.quantity).toFixed(2)}</span>
                                    <button onClick={() => onRemoveItem(item.product.id)} className="ml-2 text-red-500 hover:text-red-700"><Trash2 size={18}/></button>
                                </li>
                            ))}
                        </ul>
                        <div className="mt-6">
                            <h4 className="font-semibold mb-2 flex items-center"><MapPin className="mr-2"/> Dirección de Entrega</h4>
                            <p className="text-sm text-gray-500 mb-2">Haz clic en el mapa para ajustar tu ubicación de entrega.</p>
                        ...
                        <div className="mt-6">
                            ...
                            <textarea
                                ...
                                className="w-full p-2 border rounded-md bg-transparent dark:border-gray-600 focus:ring-orange-500 focus:border-orange-500"
                                rows={3}
                            ></textarea>
                        </div>
                    </div>
                ...
                <div className="p-4 border-t dark:border-gray-700 mt-auto">
                    ...
                    <Button onClick={handlePlaceOrderClick} className="w-full text-lg" disabled={cart.length === 0}>
                        Realizar Pedido
                    </Button>
                </div>
            </div>
